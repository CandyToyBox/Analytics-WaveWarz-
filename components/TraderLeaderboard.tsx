              ...
              {filteredTraders.map((trader, index) => (
                <tr key={trader.walletAddress} className="hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="p-4 text-right">
                    ...
                    <div className="text-[10px] text-slate-500">{formatUsd(trader.totalInvested, solPrice)}</div>
                  </td>
                  <td className="p-4 text-right">
                    <div className={`font-mono font-bold ${trader.netPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                       {trader.netPnL > 0 ? '+' : ''}{formatSol(trader.netPnL)}
                    </div>
                    <div className="text-[10px] text-slate-500">{formatUsd(trader.netPnL, solPrice)}</div>
                  </td>
